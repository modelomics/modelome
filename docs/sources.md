...
New public frontiers include Hugging Face Spaces repositories with explicit
checkpoint files, fal's paged hosted-model gallery, and a resumable Figshare OAI
model-artifact candidate history scan across one-day windows from 2011 through
2026. Its short-lived resumption tokens require prompt continuation within an
active day; expired tokens fail clearly and that day must restart. First-party checkpoint maps add OpenAI
consistency models (PyTorch and CIFAR-10 JAX), Orb Models, APT VLA,
RadiologyNET, Tencent MedicalNet, PaddleNLP ALBERT and XLM, MSST,
...
The archived KerasCV preset declarations add 79 exact public provider handles
from their source repository; internal `gs://` values are excluded.
The first-party tfhub.dev source archive supplies 2,371 versioned historical
model-card records in the current snapshot. This source preserves the archive
handle and source revision without claiming that old model bytes still resolve.
Additional first-party checkpoint inventories cover four Graphcore GPS++
...
| [Kaggle Models](https://github.com/Kaggle/kaggle-cli/blob/main/docs/models.md) | **Public `models list` API enabled with historical versions and version files.** Follow opaque page tokens for models, variations, versions, and files in `createTime` order and retain every exposed version and file reference. | Owner/model identity, framework variation, version-scoped metadata, exact file links, card/provenance/training-data links, declared license, and external base-model lineage. Also reaches models distributed through the TensorFlow/Kaggle ecosystem. | Honor provider pagination and rate responses. Per-version file metadata adds requests for each version; the public listing's reported total does not include private or unavailable models. |
