"""Enumerate archived EfficientNet checkpoint URLs from timm v0.6.13."""
...
_TAG = "v0.6.13"
_PATH = "timm/models/efficientnet.py"
_CONFIG_COUNT = 95
...
class TimmLegacyEfficientNetSourceAdapter:
    """Read literal EfficientNet URLs from one immutable timm release module."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers the 95 literal checkpoint URLs declared in timm v0.6.13's "
        "EfficientNet pretrained config. It does not enumerate current Hub files "
        "or fetch weights."
    )
    ...
    def __init__(
        self,
        *,
        name: str = "timm-legacy-efficientnet-v0613",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "timm-legacy-efficientnet-v1",
    ...
    def fetch_page(self, state: Mapping[str, Any]) -> SourcePage:
        ...
        configs = _efficientnet_urls(source_response.text(), self.name)
        records = tuple(
    ...
    def _record(
        ...
        return SourceRecord(
            source_record_id=f"legacy-efficientnet:{model_name}:v0.6.13",
...
def _efficientnet_urls(source: str, name: str) -> tuple[tuple[str, str], ...]:
    try:
    ...
    if isinstance(defaults, ast.Dict):
        pairs = list(zip(defaults.keys, defaults.values, strict=True))
    elif (
        isinstance(defaults, ast.Call)
        and isinstance(defaults.func, ast.Name)
        and defaults.func.id == "dict"
    ):
        pairs = [(ast.Constant(item.arg), item.value) for item in defaults.keywords if item.arg]
    else:
        raise ValueError(f"{name}: archived EfficientNet config has unsupported default_cfgs shape")
    ...
    for key, value in pairs:
        ...
        if url.startswith(("https://github.com/", "https://storage.googleapis.com/")):
            result.append((key.value, url))
    if len(result) != _CONFIG_COUNT:
        raise ValueError(
            f"{name}: expected {_CONFIG_COUNT} literal EfficientNet URLs; found {len(result)}"
        )
    if len({model_name for model_name, _ in result}) != len(result):
        raise ValueError(f"{name}: archived config contains duplicate EfficientNet keys")
    return tuple(result)
