"""PaddleNLP Taskflow's explicit sentiment checkpoint resource maps."""
...
_MAP_CLASSES = {"SentaTask", "SkepTask"}
_ARTIFACT_PATH = "/paddlenlp/taskflow/sentiment_analysis/"
...
def _parse_resource_maps(document: str, *, max_entries: int) -> tuple[tuple[str, str, str], ...]:
    ...
    try:
        ...
        raise ValueError("paddlenlp-taskflow-sentiment: source is not valid Python") from exc
    ...
    if {node.name for node in classes} != _MAP_CLASSES:
        raise ValueError("paddlenlp-taskflow-sentiment: expected SentaTask and SkepTask maps")
    for task_class in classes:
        ...
        if len(assignments) != 1:
            raise ValueError(f"paddlenlp-taskflow-sentiment: expected one map in {task_class.name}")
        try:
            resource_map = ast.literal_eval(assignments[0].value)
        except (ValueError, TypeError, SyntaxError, MemoryError, RecursionError) as exc:
            raise ValueError("paddlenlp-taskflow-sentiment: resource map is not literal") from exc
        if not isinstance(resource_map, dict):
            raise ValueError("paddlenlp-taskflow-sentiment: resource map is not a mapping")
        for name, resources in resource_map.items():
            ...
            if name in found and found[name] != candidate:
                raise ValueError(f"paddlenlp-taskflow-sentiment: conflicting entry for {name}")
            found[name] = candidate
            if len(found) > max_entries:
                raise ValueError(
                    f"paddlenlp-taskflow-sentiment: source exceeds {max_entries} entries"
                )
    if not found:
        raise ValueError("paddlenlp-taskflow-sentiment: no public checkpoint entries found")
    return tuple((name, *found[name]) for name in sorted(found))


class PaddleNlpTaskflowSentimentSourceAdapter:
    """Enumerate direct BILSTM/SKEP Taskflow sentiment checkpoint resources."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers public SentaTask and SkepTask model_state .pdparams resources. "
        "Excludes UIE sentiment models, internal fixtures, and auxiliary assets."
    )

    def __init__(
        self,
        *,
        name: str = "paddlenlp-taskflow-sentiment-checkpoints",
        repository: str = "PaddlePaddle/PaddleNLP",
        branch: str = "develop",
        source_path: str = "paddlenlp/taskflow/sentiment_analysis.py",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "paddlenlp-taskflow-sentiment-v1",
    ...
    def _record(self, name: str, url: str, md5: str, revision: str, digest: str) -> SourceRecord:
        identifier = Identifier("paddlenlp:taskflow-sentiment", name)
        return SourceRecord(
            ...
            text=f"PaddleNLP Taskflow sentiment checkpoint: {name}",
            ...
            releases=(
                ReleaseHint(
                    ...
                    revision=revision,
                    identifiers=(Identifier("paddlenlp:taskflow-sentiment-release", name),),
...
__all__ = ["PaddleNlpTaskflowSentimentSourceAdapter"]
