"""Enumerate RegNet checkpoint URLs from timm's archived v0.6.13 config."""
...
class TimmLegacyRegNetSourceAdapter:
    """Read literal RegNet URLs from one immutable timm release module."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers the 28 literal checkpoint URLs declared in timm v0.6.13's "
        "RegNet pretrained config. It does not enumerate current Hub files "
        "or fetch weights."
    )
    ...
    def __init__(
        self,
        *,
        name: str = "timm-legacy-regnet-v0613",
    ...
    def fetch_page(self, state: Mapping[str, Any]) -> SourcePage:
        ...
        configs = _regnet_urls(source_response.text(), self.name)
        records = tuple(
    ...
    def _record(
        ...
        return SourceRecord(
            source_record_id=f"legacy-regnet:{model_name}:v0.6.13",
...
def _regnet_urls(source: str, name: str) -> tuple[tuple[str, str], ...]:
    try:
    ...
    if isinstance(defaults, ast.Dict):
        ...
        raise ValueError(f"{name}: archived RegNet config has unsupported default_cfgs shape")
    ...
    if len(result) != _CONFIG_COUNT:
        raise ValueError(
            f"{name}: expected {_CONFIG_COUNT} literal RegNet URLs; found {len(result)}"
        )
    if len({model_name for model_name, _ in result}) != len(result):
        raise ValueError(f"{name}: archived config contains duplicate RegNet keys")
    return tuple(result)
