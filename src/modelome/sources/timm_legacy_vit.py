"""Enumerate historical ViT checkpoint URLs from timm v0.6.13."""
...
_TAG = "v0.6.13"
_PATH = "timm/models/vision_transformer.py"
_CONFIG_COUNT = 32
_URL_PREFIXES = (
    "https://github.com/",
...
class TimmLegacyViTSourceAdapter:
    """Read literal ViT URLs from one immutable timm release module."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers the 32 literal ViT checkpoint URLs declared in timm v0.6.13's "
        "Vision Transformer pretrained config. It does not enumerate current Hub "
        "files or fetch weights."
    )

    def __init__(
        self,
        *,
        name: str = "timm-legacy-vit-v0613",
    ...
    def fetch_page(self, state: Mapping[str, Any]) -> SourcePage:
        ...
        configs = _vit_urls(source_response.text(), self.name)
        records = tuple(
    ...
    def _record(
        ...
        return SourceRecord(
            source_record_id=f"legacy-vit:{model_name}:v0.6.13",
...
def _vit_urls(source: str, name: str) -> tuple[tuple[str, str], ...]:
    try:
    ...
    if isinstance(defaults, ast.Dict):
        ...
        raise ValueError(f"{name}: archived ViT config has unsupported default_cfgs shape")
    ...
    if len(result) != _CONFIG_COUNT:
        raise ValueError(
            f"{name}: expected {_CONFIG_COUNT} literal ViT URLs; found {len(result)}"
        )
    if len({model_name for model_name, _ in result}) != len(result):
        raise ValueError(f"{name}: archived config contains duplicate ViT keys")
    return tuple(result)
