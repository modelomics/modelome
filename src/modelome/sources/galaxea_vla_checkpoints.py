"""First-party Galaxea G0.5 VLA checkpoint inventory."""
...
class GalaxeaVLACheckpointSourceAdapter:
    """Enumerate the exact checkpoint folder links in Galaxea's G0.5 README table."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers only exact checkpoint folders listed in the Model Checkpoints "
        "table of OpenGalaxea/GalaxeaVLA/README.md. It does not include the "
        "historical G0/G0Plus repository or enumerate individual files within "
        "the Hub folders. Hub access requires accepting its model terms."
    )
        ...
        self.checkpoint_signature = content_hash(
            {
                ...
                "max_response_bytes": max_response_bytes,
                "max_entries": max_entries,
                "admission": "exact G05 Hugging Face folder links in Model Checkpoints table",
            }
        )
    ...
    def _record(
        ...
        model = ModelHint(
            local_id=local_id,
            name=name,
            identifiers=(identifier, Identifier("huggingface:model", "OpenGalaxea/G05")),
            aliases=(slug,),
