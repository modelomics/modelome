"""Enumerate BYOBNet checkpoint URLs from timm's archived v0.6.13 config."""
...
class TimmLegacyByobNetSourceAdapter:
    """Read literal BYOBNet URLs from one immutable timm release module."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers the 32 literal checkpoint URLs declared in timm v0.6.13's "
        "BYOBNet pretrained config. It does not enumerate current Hub files "
        "or fetch weights."
    )
    ...
    def __init__(
        self,
        *,
        name: str = "timm-legacy-byobnet-v0613",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "timm-legacy-byobnet-v1",
    ...
    def fetch_page(self, state: Mapping[str, Any]) -> SourcePage:
        ...
        configs = _byobnet_urls(source_response.text(), self.name)
        records = tuple(
    ...
    def _record(
        ...
        return SourceRecord(
            source_record_id=f"legacy-byobnet:{model_name}:v0.6.13",
...
def _byobnet_urls(source: str, name: str) -> tuple[tuple[str, str], ...]:
    try:
    ...
    if isinstance(defaults, ast.Dict):
        ...
        raise ValueError(f"{name}: archived BYOBNet config has unsupported default_cfgs shape")
    ...
    if len(result) != _CONFIG_COUNT:
        raise ValueError(
            f"{name}: expected {_CONFIG_COUNT} literal BYOBNet URLs; found {len(result)}"
        )
    if len({model_name for model_name, _ in result}) != len(result):
        raise ValueError(f"{name}: archived config contains duplicate BYOBNet keys")
    return tuple(result)
