"""Exact Keras Applications EfficientNet ImageNet weight links."""
...
_PATH = "keras/src/applications/efficientnet.py"
_MD5 = re.compile(r"^[0-9a-f]{32}$")
_VARIANT = re.compile(r"^b[0-7]$")


class KerasEfficientNetWeightsSourceAdapter:
    """Read Keras's literal EfficientNet manifest without downloading weights."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers only EfficientNet B0 through B7 ImageNet top/no-top assets "
        "declared in keras-team/keras/keras/src/applications/efficientnet.py."
    )

    def __init__(
        self,
        *,
        name: str = "keras-efficientnet-imagenet-weights",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "keras-efficientnet-weights-v1",
...
def _parse_manifest(
    ...
    try:
        ...
        raise ValueError(f"{source}: cannot parse EfficientNet source: {error.msg}") from error
    ...
    for variant, pair in sorted(hashes.items()):
        if not isinstance(variant, str) or not _VARIANT.fullmatch(variant):
            raise ValueError(f"{source}: unexpected EfficientNet variant {variant!r}")
        if (
        ...
        for include_top, suffix, checksum in (
            ...
            model_key = f"keras-applications:efficientnet:{model_name}"
            ...
            records.append(
                SourceRecord(
                    source_record_id=f"keras-efficientnet:{content_hash(url)[:24]}",
                    kind=ArtifactKind.WEIGHTS,
                    canonical_url=url,
                    title=(
                        f"Keras Applications {model_name} ImageNet weights "
                    ...
                    text=f"{model_name} ImageNet pretrained weights; include_top={include_top}.",
                    identifiers=(Identifier("keras-applications:checkpoint", url),),
                    links=(
                        Link(
                            url,
