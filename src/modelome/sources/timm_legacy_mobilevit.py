"""Enumerate historical MobileViT checkpoint URLs from timm v0.6.13."""
...
_TAG = "v0.6.13"
_PATH = "timm/models/mobilevit.py"
_CONFIG_COUNT = 16
...
class TimmLegacyMobileViTSourceAdapter:
    """Read literal MobileViT URLs from one immutable timm release module."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers the 16 literal checkpoint URLs declared in timm v0.6.13's "
        "MobileViT pretrained config. It does not enumerate current Hub files "
        "or fetch weights."
    )
    ...
    def __init__(
        self,
        *,
        name: str = "timm-legacy-mobilevit-v0613",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "timm-legacy-mobilevit-v1",
    ...
    def fetch_page(self, state: Mapping[str, Any]) -> SourcePage:
        ...
        configs = _mobilevit_urls(source_response.text(), self.name)
        records = tuple(
    ...
    def _record(
        ...
        return SourceRecord(
            source_record_id=f"legacy-mobilevit:{model_name}:v0.6.13",
...
def _mobilevit_urls(source: str, name: str) -> tuple[tuple[str, str], ...]:
    try:
    ...
    if isinstance(defaults, ast.Dict):
        ...
        raise ValueError(f"{name}: archived MobileViT config has unsupported default_cfgs shape")
    ...
    if len(result) != _CONFIG_COUNT:
        raise ValueError(
            f"{name}: expected {_CONFIG_COUNT} literal MobileViT URLs; found {len(result)}"
        )
    if len({model_name for model_name, _ in result}) != len(result):
        raise ValueError(f"{name}: archived config contains duplicate MobileViT keys")
    return tuple(result)
