"""Exact Keras Applications ConvNeXt ImageNet weight links."""
...
_PATH = "keras/src/applications/convnext.py"
_SHA = re.compile(r"^[0-9a-f]{64}$")
_VARIANT = re.compile(r"^convnext_(tiny|small|base|large|xlarge)$")


class KerasConvNeXtWeightsSourceAdapter:
    """Read the live literal ConvNeXt weight manifest without downloading weights."""

    disable_derived_extraction = True
    coverage_limitation = (
        "Covers only ConvNeXt ImageNet top and no-top weights declared in "
        "keras-team/keras/keras/src/applications/convnext.py at one commit."
    )

    def __init__(
        self,
        *,
        name: str = "keras-convnext-imagenet-weights",
        ...
        self.checkpoint_signature = content_hash(
            {
                "adapter": "keras-convnext-weights-v1",
...
def _parse_manifest(
    ...
    try:
        ...
        raise ValueError(f"{source}: cannot parse ConvNeXt source: {error.msg}") from error
    ...
    for variant, pair in sorted(hashes.items()):
        if not isinstance(variant, str) or not _VARIANT.fullmatch(variant):
            raise ValueError(f"{source}: unexpected ConvNeXt variant {variant!r}")
        if (
            not isinstance(pair, tuple)
            or len(pair) != 2
            or not all(isinstance(value, str) and _SHA.fullmatch(value) for value in pair)
        ):
            raise ValueError(f"{source}: {variant} must declare top and no-top SHA-256 hashes")
        for include_top, suffix, checksum in (
            (True, ".h5", pair[0]),
            (False, "_notop.h5", pair[1]),
        ):
            filename = variant + suffix
            url = canonicalize_url(base + filename)
            model_key = f"keras-applications:convnext:{variant}"
            ...
            title_variant = "top" if include_top else "no top"
            records.append(
                SourceRecord(
                    source_record_id=f"keras-convnext:{content_hash(url)[:24]}",
                    kind=ArtifactKind.WEIGHTS,
                    canonical_url=url,
                    title=f"Keras Applications {variant} ImageNet weights ({title_variant})",
                    raw={
                        "variant": variant,
                        "include_top": include_top,
                        "url": url,
                    ...
                    links=(
                        ...
                        Link(repository_url, relation="source_repository", crawl=False),
                    ),
                    models=(model,),
                    releases=(release,),
                )
